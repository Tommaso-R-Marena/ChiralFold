...
ChiralFold is a pure-Python scientific toolkit (library + `chiralfold` CLI + optional
Gradio web UI) for protein/peptide stereochemistry: PDB quality auditing, D-peptide
construction, AlphaFold 3 chirality correction, diastereomer enumeration, and
mirror-image (L↔D) transformation. See `README.md` and `CONTRIBUTING.md` for full usage.
...
- Console scripts (`chiralfold`, `chiralfold-web`, `pytest`, `ruff`, `mypy`) install to
...
- Web UI: `pip install -e ".[web]"` then `python web/app.py` (port 7860).
...
- Bioinformatics Overleaf one-click package:
  `paper/submission/bioinformatics/package_overleaf.sh`.
