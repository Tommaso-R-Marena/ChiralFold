...
**Ramachandran agreement with wwPDB/MolProbity** — On a seeded **era-representative** stratified sample of **363 structures** (X-ray ultra/high/med/low resolution + NMR + cryo-EM, spanning all deposition eras), produced after fixing an mmCIF→PDB chain-ID collision bug in the converter, Spearman ρ = 0.51 (95% CI [0.42, 0.60], p = 1.6 × 10⁻²⁵) on outlier percentage, with ChiralFold reporting 0.78% mean outliers vs wwPDB's 1.10%. Pearson r is 0.33 with this full set but is dominated by a single anomalous wwPDB entry (5M2K, reported as 100% outliers); excluding it, Pearson r = 0.85 and Spearman ρ = 0.52 (n = 362) — so the rank correlation is robust and the linear correlation is consistent with prior runs. This supersedes the earlier 279-structure run (ρ = 0.48, p = 2.4 × 10⁻¹⁷; r = 0.80), which used the buggy converter; the rank result is stable across both. The 104-structure run (ρ = 0.56, p = 4.4 × 10⁻¹⁰) and the original 31-structure benchmark (ρ = 0.49, p = 0.006) are retained for reference. Reproduce any run with `benchmarks/expand_ramachandran_benchmark.py` (latest outputs: `results/ramachandran_279struct_chainfix_comparison.csv`, `results/ramachandran_279struct_chainfix_summary.json`, `results/ramachandran_279struct_chainfix_plot.png`).
...
|--------|:----------:|:----------------:|:---------:|
| Ramachandran outlier % (n=363, chain-ID-fixed) | 0.78% mean | 1.10% mean | ρ = 0.51 (CI 0.42–0.60), p = 1.6×10⁻²⁵; r = 0.85 excl. 1 anomaly |
...
- Chain-ID-fixed benchmark (largest, current): 363 structures audited via seeded era-representative stratified sampling, after fixing an mmCIF→PDB chain-ID collision bug; Spearman ρ = 0.51 (95% CI [0.42, 0.60], p = 1.6×10⁻²⁵); Pearson r = 0.33 over the full set but 0.85 excluding one anomalous wwPDB entry (5M2K = 100% outliers); mean outlier rate CF 0.78% vs wwPDB 1.10%. 15 of 378 planned entries skipped: 10 lacked a wwPDB validation report, 5 had >36 distinct chains (cannot be represented in single-column PDB format). See `results/ramachandran_279struct_chainfix_summary.json`.
