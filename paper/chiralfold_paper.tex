...
ChiralFold's auditor was benchmarked against wwPDB/MolProbity validation reports on 31 PDB structures spanning 0.48--3.4~\AA\ resolution (X-ray, NMR, cryo-EM).

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{pdb_survey_figures/fig4_bland_altman}
\caption{Bland--Altman \citep{bland1986} agreement plot for ChiralFold vs wwPDB Ramachandran outlier percentages on the 31-structure benchmark. The $x$-axis is the mean of the two methods; the $y$-axis is their difference (ChiralFold $-$ wwPDB). Solid line: mean difference (\textit{Bias}~$=-0.039$\%); dashed red: 95\% limits of agreement (LoA $=$ [$-2.35$, $+2.27$]); shaded band: 95\% CI on the mean difference (\textit{Bias} 95\% CI $=$ [$-0.471$, $+0.393$]). Spearman $\rho=0.48$ ($p=0.006$), Pearson $r=0.32$ ($p=0.083$); bootstrap 95\% CI on $\rho$ is [0.13, 0.78] ($n=10{,}000$). Marker color encodes experimental method.}
\label{fig:bland_altman}
\end{figure}

Ramachandran outlier percentages showed significant agreement (Spearman $\rho=0.48$, $p=0.006$; Pearson $r=0.32$, $p=0.083$; bootstrap 95\% CI on $\rho$ = [0.13, 0.78], $n=10{,}000$), with ChiralFold reporting 0.60\% mean outliers vs wwPDB's 0.64\% (Bland--Altman mean difference $-0.039$\%, 95\% LoA [$-2.35$, $+2.27$]; \citealp{bland1986}).
...
ChiralFold combines five capabilities in a single pip-installable package: a PDB stereochemistry auditor calibrated against wwPDB/MolProbity, mathematically exact L$\leftrightarrow$D mirror-image transformation, an AF3 chirality-correction pipeline backed by an automated test suite, diastereomer enumeration for design, and interface scoring for binder evaluation. Applied to the entire PDB universe of D-amino acid residues, the auditor identifies 29 D-label/L-coordinate mismatches in 16 deposited structures --- errors invisible to existing validation pipelines --- and motivates a single concrete recommendation: the wwPDB deposition pipeline should validate the configuration of every standard-amino-acid stereocenter against the configuration encoded in the selected CCD code at deposit time. The check requires one cross product, one dot product, and one sign comparison per residue.
...
\paragraph{Ramachandran agreement (n=31 PDB structures).} Spearman $\rho=0.48$ ($p=0.006$); Pearson $r=0.32$ ($p=0.083$); OLS slope $0.225$, intercept $0.459$, $R^2=0.10$, $p=0.083$; Bland--Altman mean difference $-0.039$\% with 95\% CI [$-0.471$, $+0.393$] and 95\% limits of agreement [$-2.35$, $+2.27$]; bootstrap 95\% CI on $\rho$ = [0.13, 0.78] ($n=10{,}000$, seed 42).
