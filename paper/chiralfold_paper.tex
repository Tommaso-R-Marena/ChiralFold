...
We verified 1,677 D-amino acid residues with complete backbone atoms (N, C$_\alpha$, C, C$_\beta$) across 231 PDB files. Of these, 1,656 (98.7\%) showed correct D-chirality (negative signed volume) and 21 (1.3\%) showed L-chirality (positive signed volume) despite being labeled as D-amino acids.

The 21 errors occur in 12 PDB structures:

\begin{table}[ht]
\centering
\caption{The 12 PDB structures containing D-amino acid chirality errors. Signed volume is in \AA$^3$; positive values indicate L-stereochemistry at the C$_\alpha$.}
\label{tab:errors}
\begin{tabular}{llllllll}
\toprule
PDB ID & Residue & Chain & Position & Signed Vol.\ & Resolution & Dep.\ Date & Errors \\
\midrule
1ABI & DPN & I & 56 & $+2.49$ & 2.30~\AA & 1992 & 1 \\
1BG0 & DAR & A & 403 & $+2.58$ & 1.86~\AA & 1998 & 1 \\
1D7T & DTY & A & 4 & $+1.85$ & NMR & 1999 & 1 \\
1HHZ & DAL & E & 1 & $+2.70$ & 0.99~\AA & 2000 & 1 \\
1KO0$^\dagger$ & DLY & A & 542 & $+0.12$ & 2.20~\AA & 2001 & 1 \\
1MCB & DHI & P & 3 & $+2.60$ & 2.70~\AA & 1993 & 1 \\
1OF6 & DTY & A--H & 1369--1370 & $+2.51$ to $+2.67$ & 2.10~\AA & 2003 & 8 \\
1P52 & DAR & A & 403 & $+2.54$ & 1.90~\AA & 2003 & 1 \\
1UHG & DSN & D & 164 & $+2.21$ & 1.90~\AA & 2003 & 1 \\
1XT7 & DSG & A & 3 & $+2.55$ & NMR & 2004 & 1 \\
2AOU & DCY & A & 248 & $+2.67$ & 2.30~\AA & 2005 & 1 \\
2ATS & DLY & A & 3001--3003 & $+2.56$ to $+2.59$ & 1.90~\AA & 2005 & 3 \\
\bottomrule
\end{tabular}

\smallskip
\noindent$^\dagger$Borderline: near-zero signed volume ($+0.12$), alternate conformer B, $B$-factor 32.3~\AA$^2$. Classified as inconclusive.
\end{table}

\subsection{Robustness Verification}

To ensure the findings are not artifacts of sign convention or implementation, we performed five independent checks. First, we computed signed tetrahedron volumes for all 24 D-amino acid residues in 3IWY (experimentally confirmed D-stereochemistry); every correctly built D-residue produced a negative signed volume, confirming the convention. Second, we examined the borderline case 1KO0 DLY:542 (signed volume $+0.12$), finding that its near-zero magnitude, alternate conformer B, and moderate $B$-factor (32.3~\AA$^2$) warrant classification as inconclusive rather than a confident error. Third, we verified the 1OF6 systematic error across all 8 chains, confirming that all DTY residues use standard atom naming (N, C$_\alpha$, C, C$_\beta$) and all produce positive signed volumes ($+2.51$ to $+2.67$), with no correctly built D-residues elsewhere in the structure to serve as internal controls. Fourth, we reconfirmed the 1ABI internal control: two DPN residues in the same structure with opposite signed volumes (position~1: $-2.60$, D-correct; position~56: $+2.49$, L-error), identical atom naming, and no alternate conformations, providing a clean within-structure discrimination test. Based on these checks, we classify 11 of 12 error-containing structures as confirmed, with 1KO0 flagged as borderline. The revised count is \textbf{20 confirmed errors in 11 structures, plus 1 borderline case}.
